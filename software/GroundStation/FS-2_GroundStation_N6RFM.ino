/*
   ...
   FOSSASAT-2B Communication Guide:
...
String myCall="N6RFM";
String myGrid="FN41";
const char* mySFtext="Hello from World from N6RFM"; // optional message to be stored and forwarded.  < or = to 28 characters.
//const char* mySFhex="0x1337BEEF";  // use of mySFhex appears not correctly received as unsigned 32-bit integer, LSB first.  See ~line 1271.  Alternative to addStoreAndForward(0x1337BEEF,mySFtext); desired.
// order received is 0x13, 0x37, 0xBE, 0xEF instead mof expected oxEF, 0xBE, 0x37, 0x13.    
//////////////////////////////////////////////////////////////////////////////////////////////////
...
void printControls() {
  ...
  */
  ...
  Serial.println(F("P - get public picture (all blocks)"));
  /*Serial.println(F("F - read flash"));
  Serial.println(F("g - log GPS"));
  Serial.println(F("G - get GPS log (all blocks)"));
  */
  Serial.println(F("------------------------------------"));
  Serial.println(F("b - send preset store and forward message, defined in String mySFtext"));
  Serial.println(F("B - request store and forward message"));
  Serial.println(F("O - send route frame (retransmit, FS-2 to FS-1B)"));
  /*
...
//////////////////////////////////////////////////////////////////////////////////////////////////
///code modifided after FS-1 code from K4KDR ///
...
- Optional data:
  ...
  char optData[32];
  // msg to transmit + some additional underscores for blank space after
  String qsl = "QSL de " + myCall + " ";
  // Length (with one extra character for the null terminator)
  int str_len = qsl.length() + 1; 
  qsl.toCharArray(optData, str_len);
  
  Serial.println();
  Serial.print(F("Sending QSL ... ")); Serial.println(F("------------------------------------"));
...
- Optional data:
  ...
  char optData[32];
  // msg to transmit + some additional underscores for blank space after
  String st  = "73 de " + myCall + " ";
  // Length (with one extra character for the null terminator)
  int str_len = st.length() + 1; 
  st.toCharArray(optData, str_len);
...
///////////////////////////////////////////////////////////////////////////////////
// added additional custom commands by N6RFM

  void sendSF() {
// get message to be stored and forwarded -
 /*
 - Optional data length: 4 - 32
- Optional data:
...
*/
 addStoreAndForward(0x1337BEEF,mySFtext); //0x1337BEEF is message storage location selected by user.  Value shown is default setting.
 }
...
// - Description: Adds message to store and forward storage.
  ...
  // need to further customize how to add maniuplate ID (first 4 bytes) separately to allow user to input custom ID
  
  // get data to be retransmited
  uint8_t optData[28+4];
   optData[0] = 0x13;
   optData[1] = 0x37;
   optData[2] = 0xBE;
   optData[3] = 0xEF;
  
  Serial.println(F("Enter message to be stored and forwarded:"));
  Serial.println(F("(max 32 characters, end with LF or CR+LF)"));
  // wait for a bit to receive any trailing characters
  delay(100);
    
    uint8_t bufferPos = 4;
    while (bufferPos < 28 + 4) {
    uint8_t bufferPos = 0;
    while (bufferPos < 32) {
    while (!Serial.available());
    char c = Serial.read();
  ...
  // unsure how to manipulate the array to convert to LSB format
  ...
  // data to transmit
  uint8_t functionId = CMD_STORE_AND_FORWARD_ADD;
  uint8_t optDataLen = strlen(optData);
  ...
  // check transmission success
  if (state == ERR_NONE) {
    Serial.println(F("message for storage and forwarding ... sent successfully!"));
  } else {
    Serial.print(F("failed, code "));
    Serial.println(state);
 */
 
void Request_Public_Picture () {
   char optData[32];
   // data to transmit
  uint8_t functionId = CMD_REQUEST_PUBLIC_PICTURE;
  uint8_t optDataLen = strlen(optData);
  ...
  // check transmission success
  if (state == ERR_NONE) {
    Serial.println(F("requesting public picture ... sent successfully!"));
  } else {
    Serial.print(F("failed, code "));
    Serial.println(state);
  }
...
void setup() {
  Serial.begin(115200);
  Serial.println(F("Simplified FOSSASAT-2 Ground Station Demo Code"));
...
//        Request_Public_Picture ();
      ...
      case 'G':
        getGpsLog(1, 0, 50);
        break;
      //custom store and forward command
      case 'b':
        sendSF();
        break;
        /*
